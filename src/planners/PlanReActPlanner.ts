...
export class PlanReActPlanner extends BasePlanner {
  ...
  private splitByLastPattern(text: string, separator: string): [string, string] {
    ...
    return [text.substring(0, index + separator.length), text.substring(index + separator.length)];
  }
...
- Code must be valid self-contained TypeScript snippets with no imports and no references to tools or libraries that are not in the context.
