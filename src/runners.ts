...
export class Runner {
   ...
   *
   * @param appName The application name of the runner.
   * @param agent The root agent to run.
   * @param artifactService The artifact service for the runner.
   * @param sessionService The session service for the runner.
   * @param memoryService The memory service for the runner.
   */
  constructor(params: {
    appName: string;
    agent: BaseAgent;
  ...
  private _isTransferableAcrossAgentTree(agentToRun: BaseAgent): boolean {
    ...
    while (agent) {
      ...
      // In TypeScript we use allowTransferToPeer which is the opposite of Python's disallowTransferToParent
      // If allowTransferToPeer is explicitly set to false, return false
      if (llmAgent.allowTransferToPeer === false) {
        return false;
      }
   ...
   * @param newMessage The new message for the context.
   * @param liveRequestQueue The live request queue for the context.
   * @param runConfig The run config for the context.
  ...
  private _newInvocationContext(params: {
    ...
    if (runConfig.supportCfc && this.agent instanceof LlmAgent) {
      // Get the agent's model name - in TypeScript we might access it differently
      const llmAgent = this.agent as LlmAgent;
      const modelName = (llmAgent.llm as any)?.model || 'unknown';
