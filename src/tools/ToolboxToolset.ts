...
import { SimpleToolboxClient, ToolboxClient } from './ToolboxTool';
...
export class ToolboxToolset extends BaseToolset {
  ...
  async getTools(readonlyContext?: ReadonlyContext): Promise<BaseTool[]> {
    ...
    if (this.toolset_name) {
      const toolset = await this.toolboxClient.loadToolset(this.toolset_name);
      tools.push(
    ...
    if (this.tool_names) {
      const loadedTools = await Promise.all(
        this.tool_names.map((toolName) =>
          this.toolboxClient.loadTool(toolName),
        ),
      );
      tools.push(
  ...
  async close(): Promise<void> {
    // The SimpleToolboxClient does not have a close method.
